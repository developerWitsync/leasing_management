                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($life_of_asset->years > 0)
                                                    <a data-href="{{ route('settings.leaseassets.editlife', ['id' => $life_of_asset->id]) }}" href="javascript:;" class="btn btn-sm btn-success edit_table_setting">
                                                        <i class="fa fa-pencil-square-o"></i>
                                                    </a>
                                                    <a data-href="{{ route('settings.leaseassets.deletelife', ['id' => $life_of_asset->id]) }}" href="javascript:;" class="btn btn-sm btn-danger delete_settings"><i class="fa fa-trash-o"></i></a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                                        ...
                                        <td>
                                            <form action="{{ route('settings.leaseassets.addlife') }}" method="POST" class="add_more_expected_life_of_assets_form">
                                                ...
                                                <div class="form-group{{ $errors->has('expected_life_number') ? ' has-error' : '' }}">
                                                    <input type="text" value="{{ old('expected_life_number') }}" name="expected_life_number" placeholder="Expected Useful Life In Years" class="form-control {{ $errors->has('expected_life_number') ? ' has-error' : '' }}"/>
            ...
            $(document).ready(function () {
                var table_{{ $category->id }} = $('.{{ str_slug($category->title) }}').DataTable({
                    ...
                    "columns": [
                        ...
                        { "data": "title"},
                        { "data": "depreciation_method.title", 'sortable': false},
                        { "data": "id"}
                    ],
                    "columnDefs": [
                        {
