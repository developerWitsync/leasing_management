                    ...
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <form class="form-horizontal" method="POST"
                                  action="{{ route('settings.user.assigned-permission-role', ['id' => $role->id])}}">
                                {{ csrf_field() }}
                                        ...
                                        @foreach($permission as $permission)
                                            <div class="col-md-12">
                                                <input type="checkbox" name="permission[]"
                                                       @if(old('permission',in_array($permission->id,$PermissionRoleId))) checked="checked"
                                                       @endif @if($permission->id == "2") disabled="disabled"
                                                       @endif value="{{$permission->id}}">{{ $permission->display_name }}
                                            </div>
                                        @endforeach
                                ...
                                <div class="form-group">
                                    <div class="col-md-6 col-md-offset-4">
                                        ...
                                        <a href="{{route('settings.role')}}" class="btn btn-sm btn-danger add_more"
                                           data-form="add_more_form_lease_basis">Cancel</a>
                                    </div>
                                </div>
                            </form>
                        </div>
