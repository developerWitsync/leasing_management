<form role="form" class="form-horizontal" method="post" enctype="multipart/form-data">
    ...
    <div class="categoriesOuter clearfix immediate_previous_lease_end_date" style="@if(old('payout_time', $payment->payout_time) == 2) @else display: none @endif">
        ...
        <div class="form-group{{ $errors->has('immediate_previous_lease_end_date') ? ' has-error' : '' }} required">
            <label for="immediate_previous_lease_end_date" class="col-md-12 control-label" style="display: inline">Immediate Previous Lease End Date</label>
            {!! renderToolTip('Please select the date at which the first lease interval starts, i.e., the date from which the lease accrual period starts.
...
Example: If the Lease Interval is quarterly, Lease payment made at the end of the interval and the first lease payment made on March 31, 20XX, so the first lease interval start date should be January 01, 20XX.', 'input_info_tooltip', 'top' , 'display:inline;') !!}
            <div class="col-md-12">
    ...
    <div class="categoriesOuter clearfix last_lease_payment_interval_end_date" style="@if(old('payout_time', $payment->payout_time) == 2) display: none @else  @endif">
        ...
        <div class="form-group{{ $errors->has('last_lease_payment_interval_end_date') ? ' has-error' : '' }} required">
            <label for="last_lease_payment_interval_end_date" class="col-md-12 control-label" style="display: inline">Last Lease Payment Interval End Date</label>
            {!! renderToolTip('Please select the date at which the last lease advance payment interval ends, i.e., the date at which the lease interval advance gets fully amortized. This date is required for auto-computation of lease expenses in cases where the lease payment not qualify for capitalization as per conditions of the New Lease Accounting Standard. Example: If the Lease Interval is quarterly, Lease payment made at the start of the interval and the last lease advance payment made on January 01, 20XX, so the last lease payment interval should end on March 31, 20XX.', 'input_info_tooltip', 'top' , 'display:inline;') !!}
