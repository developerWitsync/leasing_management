...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `present_value_of_lease_liability` (IN `payment_year` INT, IN `payment_month` INT, IN `base_date` DATE, IN `asset_id` INT, IN `payment_id` INT)  BEGIN
    ...
    ELSE 
		...
		select SUM(`lease_assets_payments`.`total_amount_per_interval`) into total_amount_payable from `lease_asset_payment_dates` 
