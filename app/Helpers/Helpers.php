...
/**
 * Returns the Ledgers Data for the Main Category or Sub-Category as well....
 * @param $id
 * @return array
 */
function getLedgersData($id)
{
  $general_settings = \App\GeneralSettings::query()->whereIn('business_account_id', getDependentUserIds())->first();

  if ($general_settings->ledger_level == 1) {
    $category = \App\LeaseAssetCategories::query()->find($id);
  } elseif ($general_settings->ledger_level == 2) {
    $category = \App\LeaseAssetSubCategorySetting::query()->whereIn('business_account_id', getDependentUserIds())
      ->where('id', '=', $id)->first();
  }

  $ledgers = \App\Ledgers::query()->whereIn('business_account_id', getDependentUserIds())
